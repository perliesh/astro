            ...
            if image_hdu is None:
                ...
                if 'RA' in header and 'DEC' in header:
                    try:
                        ...
                        # 문자열인 경우: hh:mm:ss 형식이면 hourangle 처리
                        if isinstance(ra_raw, str) or isinstance(dec_raw, str):
                            target_coord = SkyCoord(ra_raw, dec_raw, unit=('hourangle', 'deg'))
                        else:
                            # 숫자(float)인 경우는 그냥 deg로 처리
                            target_coord = SkyCoord(ra=ra_raw * u.deg, dec=dec_raw * u.deg, frame='icrs')
